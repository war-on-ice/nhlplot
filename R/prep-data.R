...
produce.hexshots <- function (input.data) {
    ...
    shotdata <- filter (subdata, !is.na(xcoord),
                        etype %in% c("GOAL","MISS","SHOT","BLOCK")) %>%
                            mutate (hexblock = hexbin.quick (cbind(newyc, newxc), scatter.grid))
    ...
    sub.bit <- function (df, colname) {
        extract <- df[,c(colname, "season", "gcode", "etype",
    ...
    reduced.shots <- rbind_list (sub.bit(home.events, "h1") %>% mutate (home=1, role="O"),
                                 ...
                                 sub.bit(home.events, "a1") %>% mutate (home=1, role="D"),
                                 sub.bit(home.events, "a2") %>% mutate (home=1, role="D"),
                                 sub.bit(home.events, "a3") %>% mutate (home=1, role="D"),
                                 sub.bit(home.events, "a4") %>% mutate (home=1, role="D"),
                                 sub.bit(home.events, "a5") %>% mutate (home=1, role="D"),
                                 sub.bit(home.events, "a6") %>% mutate (home=1, role="D"),
                                 sub.bit(home.events, "away.G") %>% mutate (home=1, role="G"),
                                 sub.bit(home.events, "ev.player.1") %>% mutate (home=1, role="S"),
                                 
                                 sub.bit(away.events, "h1") %>% mutate (home=0, role="D"),
                                 sub.bit(away.events, "h2") %>% mutate (home=0, role="D"),
                                 sub.bit(away.events, "h3") %>% mutate (home=0, role="D"),
                                 sub.bit(away.events, "h4") %>% mutate (home=0, role="D"),
                                 sub.bit(away.events, "h5") %>% mutate (home=0, role="D"),
                                 sub.bit(away.events, "h6") %>% mutate (home=0, role="D"),
                                 sub.bit(away.events, "home.G") %>% mutate (home=0, role="G"),
